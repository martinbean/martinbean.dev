...
Using roles can sound scary and complicated but it doesn’t have to be. It can something a simple a string with the name of an abitrary role (i.e. “admin” or “manager”). You can also easily add single role and multiple roles to users in your Laravel applications. Below are two simple approaches to both single roles and multiple roles.
...
### Migration
```
php artisan make:migration add_role_column_to_users_table --table=user
```
```php
<?php
...
**Tip:** Any time you’re dealing with a `Request` object you can get the authenticated user without resorting to the `Auth` facade or `auth()->user()` helper. Just call `$request->user()` and that will return the authentication user. You can do this in controllers as well.
...
First, create the `Role` model and a corresponding migration:
...
## Conclusion
So, next to you’re dealing with an application with multiple user types, consider using roles instead of splitting your users across multiple models and tables. It’ll make users in your application much easier to deal with!
